...
import { TableHeader, StudentLessonTableItem } from '~/types/props/shift'
...
export default defineComponent({
  ...
  setup(props, { emit }: SetupContext) {
    const headers: TableHeader[] = [
      ...
      { text: '講師', value: 'teacherId', align: 'center' },
    ]
    ...
    const getItems = (): StudentLessonTableItem[] => {
      const format: string = 'DD(ddd)'
      const items: StudentLessonTableItem[] = props.lesson.lessons.map((lesson: ShiftLesson) => {
        const date: string = dayjs(lesson.startAt).tz().format(format)
        const duration: string = getDuration(lesson.startAt, lesson.endAt)
        return { date, duration, subjectId: lesson.subjectId, teacherId: lesson.teacherId }
      })
      return items
    }
