...
import { StudentUpdatePasswordOptions, StudentUpdatePasswordParams } from '~/types/form'
...
export default defineComponent({
  ...
  setup() {
    ...
    const updatePasswordForm = reactive({
      params: StudentUpdatePasswordParams,
      options: StudentUpdatePasswordOptions,
    })

    const handleSubmit = async (): Promise<void> => {
      CommonStore.startConnection()

      await AuthStore.updatePassword({ form: updatePasswordForm })
        .then(() => {
          AuthStore.signOut()
          router.push('/signin')
