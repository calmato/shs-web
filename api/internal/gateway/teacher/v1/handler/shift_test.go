...
func TestListShifts(t *testing.T) {
	...
	teacherSubmissions := []*lesson.TeacherSubmission{
		{
			TeacherId:      "teacherid",
			ShiftSummaryId: 1,
			Decided:        true,
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(ctx context.Context, t *testing.T, mocks *mocks, ctrl *gomock.Controller) {
				...
				shiftsOut := &lesson.ListShiftsResponse{Shifts: shifts}
				teacherSubmissionsIn := &lesson.ListTeacherSubmissionsByTeacherIDsRequest{
					TeacherIds: []string{"teacherid"}, ShiftSummaryId: 1,
				}
				teacherSubmissionsOut := &lesson.ListTeacherSubmissionsByTeacherIDsResponse{Submissions: teacherSubmissions}
				...
				mocks.lesson.EXPECT().ListShifts(gomock.Any(), shiftsIn).Return(shiftsOut, nil)
				mocks.lesson.EXPECT().ListTeacherSubmissionsByTeacherIDs(gomock.Any(), teacherSubmissionsIn).
					Return(teacherSubmissionsOut, nil)
...
func TestListShiftLessons(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			expect: &testResponse{
				code: http.StatusOK,
				body: &response.ShiftLessonsResponse{
					Lessons: entity.Lessons{
						{
							ID:        1,
							ShiftID:   1,
