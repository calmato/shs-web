...
func newMock() (*mocks, error) {
	...
	conf := &firebase.Config{ProjectID: "project-test"}
	app, err := firebase.NewApp(ctx, conf)
	if err != nil {
		return nil, err
	}
	fa, err := authentication.NewClient(ctx, app)
	if err != nil {
		return nil, err
	}

	return &mocks{db: db, auth: fa}, nil
}
...
func (m *mocks) authDelete(ctx context.Context, ids ...string) error {
	for _, id := range ids {
		if err := m.auth.DeleteUser(ctx, id); err != nil {
			return err
		}
	}
	return nil
